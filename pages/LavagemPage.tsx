...
import type { User, LavagemPackage, LavagemClient, LavagemRecord, FinancialTransaction, FinancialCategory, LavagemContract } from '../types';
...
type PendingAction = {
    type: 'execute' | 'delete' | 'change_date';
...
const LavagemPage: React.FC<{ currentUser: User }> = ({ currentUser }) => {
    ...
    // Filtros de Período Solicitados
    ...
    const [editWashDateValue, setEditWashDateValue] = useState('');
    ...
    // --- LÓGICA DE FILTRAGEM DE CLIENTES (USADA PARA A LISTA E PARA O CÁLCULO) ---
    const visibleClientsWithInfo = useMemo(() => {
        ...
        return clients.map(client => ({
            ...
            // Se estiver na aba de oportunidades, filtra quem NÃO tem pacote
            if (activeTab === 'oportunidades' && client.package_id) return false;
            // Se estiver na aba de timeline, filtra quem TEM pacote
    ...
    // --- CÁLCULO DO VALOR ARRECADADO (BASEADO NOS CLIENTES FILTRADOS) ---
    const totalArrecadado = useMemo(() => {
        const currentYear = new Date().getFullYear();
        const visibleClientIds = new Set(visibleClientsWithInfo.map(item => item.client.id));

        return contracts.reduce((sum, contract) => {
            // Só considera se o cliente está na lista atual (respeita Busca e Status)
            ...
            if (usePeriodFilter) {
                // Se período habilitado, filtra pela data do contrato
                if (contractDate >= periodStart && contractDate <= periodEnd) {
                    return sum + Number(contract.total_value);
                }
            } else {
                // Se desativado, mostra o acumulado do ano atual dos clientes filtrados
    ...
    const timelineGroups = useMemo(() => {
        ...
        return Object.keys(groups)
            .sort((a, b) => a === 'pending' ? -1 : a.localeCompare(b))
    ...
    const executePendingAction = async () => {
        ...
        try {
            ...
            else if (pendingAction.type === 'change_date') await dataService.save('lavagem_records', { ...pendingAction.record, date: pendingAction.payload });
            await loadData();
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-24">
                ...
                <div className="bg-indigo-50 dark:bg-indigo-900/30 p-4 rounded-2xl border-2 border-indigo-100 dark:border-indigo-800 flex items-center gap-4 shadow-sm min-w-[260px]">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest leading-none mb-1">
                            {usePeriodFilter ? 'Arrecadado no período' : `Arrecadado em ${new Date().getFullYear()}`}
                        </p>
                        <p className="text-xl font-black text-indigo-700 dark:text-indigo-300 leading-none">
                            {formatCurrency(totalArrecadado)}
                        </p>
                        <p className="text-[8px] text-indigo-400 font-bold mt-1 uppercase tracking-tighter">* Inclui histórico completo da lista atual</p>
                    </div>
                </div>
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="bg-white dark:bg-gray-800 p-4 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col lg:flex-row items-center gap-4">
                    <div className="relative flex-1 w-full">
                        ...
                        <input type="text" placeholder="Buscar cliente por nome..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-11 pr-4 py-3 rounded-2xl bg-gray-50 dark:bg-gray-900 border-transparent text-sm font-bold outline-none focus:ring-2 focus:ring-cyan-500/20 transition-all" />
                    </div>
                    
                    <div className="flex flex-wrap items-center gap-4 w-full lg:w-auto">
                        <div className="min-w-[140px]">
                            <FormLabel>Situação</FormLabel>
                            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="w-full bg-gray-50 dark:bg-gray-900 border-none rounded-xl p-2.5 text-[11px] font-bold dark:text-white outline-none appearance-none cursor-pointer">
                                <option value="Todos">Todas situações</option>
                                <option value="Oportunidade 1 Ano">Oportunidade 1 Ano 🔥</option>
                                <option value="Atrasados">Em atraso ⚠️</option>
                                <option value="Com agendamentos">Com agendamentos</option>
                                <option value="Sem Agendamento">Sem agendamento</option>
                                <option value="Ciclo Finalizado">Ciclo Finalizado</option>
                            </select>
                        </div>

                        <div className="flex items-end gap-3 bg-gray-50 dark:bg-gray-900/40 p-2 rounded-2xl border border-gray-100 dark:border-gray-700">
                            <div><FormLabel>Período</FormLabel><select value={usePeriodFilter ? 'enabled' : 'disabled'} onChange={e => setUsePeriodFilter(e.target.value === 'enabled')} className="bg-white dark:bg-gray-800 border-none rounded-lg px-3 py-1.5 text-[11px] font-bold outline-none cursor-pointer shadow-sm"><option value="disabled">Desativado</option><option value="enabled">Habilitado</option></select></div>
                            <div className={`flex items-center gap-2 transition-opacity ${usePeriodFilter ? 'opacity-100' : 'opacity-30 pointer-events-none'}`}><div className="relative"><CalendarIcon className="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" /><input type="date" value={periodStart} onChange={e => setPeriodStart(e.target.value)} className="bg-white dark:bg-gray-800 border-none rounded-lg pl-8 pr-2 py-1.5 text-[11px] font-bold outline-none shadow-sm" /></div><span className="text-gray-300 font-bold">-</span><div className="relative"><CalendarIcon className="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" /><input type="date" value={periodEnd} onChange={e => setPeriodEnd(e.target.value)} className="bg-white dark:bg-gray-800 border-none rounded-lg pl-8 pr-2 py-1.5 text-[11px] font-bold outline-none shadow-sm" /></div></div>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    {timelineGroups.map(group => (
                        <section key={group.key} className="relative">
                            ...
                            <div className="flex items-center gap-4 mb-6 relative z-10">
                                <div className={`w-12 h-12 rounded-full flex items-center justify-center border-4 border-white dark:border-gray-950 shadow-md ${group.key === 'pending' ? 'bg-slate-500 text-white' : 'bg-cyan-600 text-white'}`}>{group.key === 'pending' ? <ClockIcon className="w-6 h-6" /> : <CalendarIcon className="w-6 h-6" />}</div>
                                <h3 className="text-lg font-black text-gray-800 dark:text-white tracking-tight">{group.label} <span className="ml-2 text-xs font-bold text-gray-400 bg-gray-50 dark:bg-gray-800 px-2 py-0.5 rounded-lg border">{group.items.length}</span></h3>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ml-0 lg:ml-16">
                                {group.items.map(({ client, info }) => {
                                    ...
                                    return (
                                        <div key={client.id} className={`bg-white dark:bg-gray-800 rounded-3xl border p-5 shadow-sm hover:shadow-xl transition-all group overflow-hidden flex flex-col relative ${hasOverdue ? 'border-red-200 dark:border-red-900/50 ring-2 ring-red-500/10' : 'border-gray-100 dark:border-gray-700'}`}>
                                            ...
                                            <div className="flex justify-between items-start mb-4">
                                                <div className="space-y-0.5"><div className="flex items-center gap-1.5"><h4 className="font-black text-gray-800 dark:text-white text-base leading-tight">{toSentenceCase(client.name)}</h4>{info.totalAccumulated > info.contractTotalValue && <StarIcon className="w-3 h-3 text-amber-500 fill-amber-500" title="Cliente Fiel" />}</div><p className="text-[10px] text-gray-400 font-black flex items-center gap-1"><MapPinIcon className="w-3 h-3" /> {client.city || 'S/ local'} • {client.plates_count} placas</p></div>
                                            ...
                                            <div className="flex-1 space-y-4">
                                                {(info.allScheduled.length > 0 || info.pendingToScheduleCount > 0) ? (
                                                    <div style={{ backgroundColor: packageColor }} className="rounded-2xl text-white shadow-lg shadow-black/5 overflow-hidden">
                                                        <div className="bg-black/10 px-4 py-2 border-b border-white/10 flex justify-between items-center"><p className="text-[9px] font-black opacity-80 tracking-wide">Visitas agendadas</p><span className="text-[9px] font-black bg-white/20 px-1.5 rounded">{info.allScheduled.length} / {info.washQtyLimit - info.executedCount}</span></div>
                                                            ...
                                                            {info.allScheduled.map((record, idx) => (
                                                                <div key={record.id} className={`relative bg-white/10 hover:bg-white/20 p-1.5 rounded-lg border transition-all flex flex-col items-center justify-center text-center gap-1 ${idx === 0 ? 'border-white border-2 scale-105 shadow-md' : 'border-white/10'}`}>
                                                                    ...
                                                                    <div className="flex items-center gap-1 mt-0.5">
                                                                        <button onClick={() => { setPendingAction({ type: 'execute', record: record }); setIsConfirmActionModalOpen(true); }} className="p-0.5 bg-white text-indigo-600 rounded-md shadow-sm hover:scale-110 transition-all" style={{ color: packageColor }}><CheckCircleIcon className="w-3 h-3" /></button>
                                                                        <button onClick={() => { setPendingAction({ type: 'change_date', record: record }); setEditWashDateValue(record.date); setIsEditWashDateModalOpen(true); }} className="p-0.5 bg-black/20 text-white rounded-md hover:scale-110 transition-all"><EditIcon className="w-3 h-3" /></button>
                                                                    </div>
                                                                </div>
                                                            ))}
                                                    ...
                                                    <div className="p-4 bg-slate-50 dark:bg-slate-900/10 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl text-center"><p className="text-[10px] font-black text-slate-700 mb-2">{isFinished ? 'Ciclo finalizado ✅' : 'Aguardando agenda'}</p><button onClick={() => isFinished ? handleOpenLaunchModal(client.id) : (() => { setSelectedClient(client); const init = Array(info.washQtyLimit - info.executedCount).fill(new Date().toISOString().split('T')[0]); setScheduleDates(init); setFrequency('semestral'); setIsWashModalOpen(true); })()} className={`w-full py-2 rounded-xl font-black text-[9px] tracking-tight shadow-md ${isFinished ? 'bg-indigo-600 text-white' : 'bg-slate-500 text-white'}`}>{isFinished ? 'Renovar agora' : 'Gerar cronograma'}</button></div>
                                                )}
                                                <div className="space-y-1.5 pt-2">
                                                    <div className="flex justify-between items-end px-1">
                                                        <div className="flex flex-col"><span className="text-[9px] font-black text-gray-400 tracking-tighter uppercase">{info.pkg?.name || '---'}</span><button onClick={() => { setSelectedClient(client); setIsContractDetailModalOpen(true); }} className="text-[11px] font-black text-indigo-600 hover:text-indigo-800 transition-colors flex items-center gap-1">{formatCurrency(info.contractTotalValue)}<ChevronDownIcon className="w-2.5 h-2.5 opacity-50" /></button></div>
                                                        <div className="flex flex-col text-right"><span className="text-[8px] font-black text-gray-400 uppercase">Inv. Total</span><button onClick={() => { setSelectedClient(client); setIsContractDetailModalOpen(true); }} className={`text-[10px] font-black flex items-center justify-end gap-1 transition-all ${info.totalAccumulated > 0 ? 'text-amber-600 hover:text-amber-700' : 'text-gray-400'}`}>{formatCurrency(info.totalAccumulated)}<DocumentReportIcon className="w-2.5 h-2.5 opacity-60" /></button></div>
                                                    </div>
                                                    <div className="w-full h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden"><div className="h-full transition-all duration-700 rounded-full" style={{ width: `${percent}%`, backgroundColor: packageColor }} /></div>
                                                </div>
                                            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in">
                    {visibleClientsWithInfo.map(({ client, info }) => {
                        ...
                        if (client.installation_end_date) {
                            ...
                            const instDate = new Date(client.installation_end_date);
                            const diffTime = Math.abs(today.getTime() - instDate.getTime());
                            const diffDays = Math.ceil(today.getTime() - instDate.getTime()) / (1000 * 60 * 60 * 24);
                            if (diffDays >= 365) {
                                const years = Math.floor(diffDays / 365);
                                const months = Math.floor((diffDays % 365) / 30);
                                timeText = `${years} ano${years > 1 ? 's' : ''}${months > 0 ? ` e ${months} m` : ''}`;
                            } else {
                                const months = Math.floor(diffDays / 30);
                                timeText = months > 0 ? `${months} meses` : `${Math.floor(diffDays)} dias`;
                            }
                        }
                        ...
                        return (
                            <div key={client.id} className={`bg-white dark:bg-gray-800 rounded-3xl border-2 p-5 shadow-sm hover:shadow-xl transition-all flex flex-col relative ${isHot ? 'border-amber-400 dark:border-amber-600 ring-4 ring-amber-500/5' : 'border-gray-100 dark:border-gray-700'}`}>
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    <div className="space-y-1"><h4 className="font-black text-gray-800 dark:text-white text-base leading-tight">{toSentenceCase(client.name)}</h4><p className="text-[10px] text-gray-400 font-black flex items-center gap-1"><MapPinIcon className="w-3 h-3" /> {client.city || 'S/ local'}</p></div>
                                    <button onClick={() => { setEditingClient(client); setClientForm({...client, installation_end_date: client.installation_end_date || ''}); setIsClientModalOpen(true); }} className="p-1.5 text-gray-300 hover:text-cyan-600"><EditIcon className="w-4 h-4" /></button>
                                </div>
                                <div className="flex-1 space-y-4">
                                    <div className={`p-4 rounded-2xl border-2 border-dashed ${isHot ? 'bg-amber-50 dark:bg-amber-900/10 border-amber-300' : 'bg-gray-50 dark:bg-gray-900/10 border-gray-200'} text-center`}>
                                        <div className="flex items-center justify-center gap-2 mb-1"><ClockIcon className={`w-4 h-4 ${isHot ? 'text-amber-600' : 'text-gray-400'}`} /><span className={`text-[10px] font-black uppercase tracking-tighter ${isHot ? 'text-amber-600' : 'text-gray-500'}`}>Tempo de Instalação</span></div>
                                        <p className={`text-xl font-black ${isHot ? 'text-amber-700 dark:text-amber-400' : 'text-gray-600'}`}>{timeText}</p>
                                        <p className="text-[9px] text-gray-400 font-bold mt-1">Data: {client.installation_end_date ? new Date(client.installation_end_date).toLocaleDateString('pt-BR', {timeZone:'UTC'}) : '---'}</p>
                                    </div>
                                    <div className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-xl flex justify-between items-center border border-gray-100 dark:border-gray-700">
                                        <div className="flex flex-col"><span className="text-[9px] font-bold text-gray-400">Potencial</span><span className="text-xs font-black text-gray-700 dark:text-gray-200">{client.plates_count} Placas</span></div>
                                        <div className="w-px h-6 bg-gray-200 dark:bg-gray-700"></div>
                                        <div className="flex flex-col text-right"><span className="text-[9px] font-bold text-gray-400">Status</span><span className={`text-[10px] font-black ${isHot ? 'text-amber-600' : 'text-gray-500'}`}>{isHot ? 'HOT 🔥' : 'Holding ❄️'}</span></div>
                                    </div>
                                    <button onClick={() => handleOpenLaunchModal(client.id)} className={`w-full py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg transition-all ${isHot ? 'bg-amber-500 hover:bg-amber-600 text-white shadow-amber-500/20' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400'}`}>Lançar Plano</button>
                                </div>
                            </div>
                        );
                    })}
            ...
            {/* Modal de Ficha Financeira */}
            {isContractDetailModalOpen && selectedClient && (() => {
                const info = getClientPackageInfo(selectedClient);
                return (
                    <Modal title="Ficha financeira do cliente" onClose={() => setIsContractDetailModalOpen(false)} maxWidth="max-w-2xl">
                        <div className="space-y-6 pt-2">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="p-4 bg-indigo-50 dark:bg-indigo-900/30 rounded-2xl border-2 border-indigo-100 dark:border-indigo-800">
                                    <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest leading-none mb-2">Plano Ativo</p>
                                    <h4 className="text-base font-black text-indigo-700 dark:text-indigo-400 leading-tight">{info.pkg?.name || '---'}</h4>
                                    <p className="text-[11px] text-indigo-600 dark:text-indigo-500 font-bold mt-1 tracking-tight">Ciclo atual iniciado: {selectedClient.package_launch_date ? new Date(selectedClient.package_launch_date).toLocaleDateString('pt-BR') : '---'}</p>
                                    <div className="mt-3 pt-3 border-t border-indigo-200 dark:border-indigo-800 flex flex-col gap-1">
                                        <div className="flex justify-between items-center text-[10px] font-bold text-indigo-400"><span>Deslocamento atual</span><span>{formatCurrency(info.travelCost)}</span></div>
                                        <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-indigo-400">Valor do Contrato</span><span className="text-lg font-black text-indigo-700 dark:text-indigo-300">{formatCurrency(info.contractTotalValue)}</span></div>
                                    </div>
                                </div>
                                <div className="p-4 bg-amber-50 dark:bg-amber-900/30 rounded-2xl border-2 border-amber-100 dark:border-amber-800 flex flex-col justify-between">
                                    <div><p className="text-[10px] font-black text-amber-500 uppercase tracking-widest leading-none mb-2">Relacionamento</p><div className="flex items-center gap-2"><h4 className="text-base font-black text-amber-700 dark:text-amber-500 leading-tight">Investimento Acumulado</h4>{info.totalAccumulated > info.contractTotalValue && <StarIcon className="w-4 h-4 text-amber-500 fill-amber-500" />}</div></div>
                                    <div className="text-right"><p className="text-3xl font-black text-amber-600 dark:text-amber-400">{formatCurrency(info.totalAccumulated)}</p></div>
                                </div>
                            </div>
                            <div className="space-y-4">
                                <h4 className="text-[11px] font-black text-gray-400 uppercase tracking-widest border-b pb-2 flex items-center gap-2"><DocumentReportIcon className="w-4 h-4 text-indigo-500" /> Histórico de Planos Fechados</h4>
                                <div className="space-y-2 max-h-[250px] overflow-y-auto pr-2 custom-scrollbar">
                                    {info.clientContracts.length > 0 ? info.clientContracts.map((contract) => (
                                        <div key={contract.id} className="flex items-center justify-between p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm hover:border-amber-200 transition-all group">
                                            <div className="flex items-center gap-3"><div className="p-2.5 bg-green-50 dark:bg-green-900/30 text-green-600 rounded-xl shadow-sm"><CheckCircleIcon className="w-5 h-5" /></div><div><p className="text-xs font-black text-gray-800 dark:text-white leading-tight">{contract.package_name}</p><div className="flex items-center gap-2 mt-1"><span className="text-[9px] font-bold text-gray-400 uppercase">Fechamento:</span><span className="text-[10px] font-black text-indigo-600">{new Date(contract.created_at).toLocaleDateString('pt-BR')}</span></div></div></div>
                                            <div className="text-right"><p className="text-xs font-black text-green-600">{formatCurrency(Number(contract.total_value))}</p><span className="text-[8px] font-black bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded text-gray-400 uppercase tracking-tighter">Liquidado</span></div>
                                        </div>
                                    )) : (<div className="text-center py-10 bg-gray-50 dark:bg-gray-900/40 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-800"><p className="text-xs font-bold text-gray-400 italic">Nenhum contrato histórico registrado para este cliente.</p></div>)}
                                </div>
                            </div>
                            <button onClick={() => setIsContractDetailModalOpen(false)} className="w-full py-4 bg-gray-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl transition-all active:scale-95">Fechar Detalhamento</button>
                        </div>
                    </Modal>
                );
            })()}
            ...
            {/* Demais Modais */}
            {isEditWashDateModalOpen && pendingAction && (<Modal title="Alterar data" onClose={() => setIsEditWashDateModalOpen(false)} maxWidth="max-w-sm"><div className="space-y-4 pt-2"><div><FormLabel>Nova data da visita</FormLabel><input type="date" value={editWashDateValue} onChange={(e) => setEditWashDateValue(e.target.value)} className="w-full rounded-xl border-gray-300 bg-gray-50 p-3 text-sm font-black shadow-sm" /></div><div className="flex gap-3 pt-4 border-t"><button onClick={() => setIsEditWashDateModalOpen(false)} className="flex-1 py-3 bg-gray-100 text-gray-500 rounded-xl font-bold text-xs">Cancelar</button><button onClick={() => { setPendingAction({ ...pendingAction, payload: editWashDateValue }); executePendingAction(); }} className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-black text-xs shadow-lg">Salvar</button></div></div></Modal>)}
            ...
            {isHistoryModalOpen && selectedClient && (<Modal title={`Histórico: ${selectedClient.name}`} onClose={() => setIsHistoryModalOpen(false)} maxWidth="max-w-2xl"><div className="space-y-6 pt-2"><div className="p-5 bg-gray-50 dark:bg-gray-900/50 rounded-3xl border border-gray-100 dark:border-gray-800 space-y-2 shadow-inner"><p className="text-sm font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2"><MapPinIcon className="w-4 h-4 text-cyan-600" /> {selectedClient.address}, {selectedClient.address_number} - {selectedClient.city}</p><p className="text-xs font-black text-cyan-600">{selectedClient.plates_count} placas no sistema</p></div><div className="space-y-3"><h4 className="text-[11px] font-black text-gray-400 px-1 tracking-tight">Histórico de visitas</h4><div className="space-y-2.5 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">{records.filter(r => r.client_id === selectedClient.id).sort((a,b) => b.date.localeCompare(a.date)).map((record) => (<div key={record.id} className="flex items-center justify-between p-4 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm"><div className="flex items-center gap-4"><div className={`p-2.5 rounded-xl ${record.status === 'executed' ? 'bg-emerald-100 text-emerald-600' : 'bg-cyan-100 text-cyan-600'}`}>{record.status === 'executed' ? <CheckCircleIcon className="w-5 h-5" /> : <ClockIcon className="w-5 h-5" />}</div><div><p className="text-xs font-black text-gray-800 dark:text-white">Lavagem - {new Date(record.date).toLocaleDateString('pt-BR', {timeZone: 'UTC'})}</p><p className={`text-[9px] font-bold ${record.status === 'executed' ? 'text-emerald-600' : 'text-cyan-600'}`}>{record.status === 'executed' ? 'Concluída' : 'Agendada'}</p></div></div><div className="flex gap-1.5"><button onClick={() => { setPendingAction({ type: 'delete', record: record }); setIsConfirmActionModalOpen(true); }} className="p-2 text-gray-400 hover:text-red-600 bg-gray-50 dark:bg-gray-700 rounded-lg"><TrashIcon className="w-4 h-4"/></button></div></div>))}{records.filter(r => r.client_id === selectedClient.id).length === 0 && (<p className="text-center py-10 text-gray-400 text-xs italic font-bold">Nenhuma visita registrada para este cliente.</p>)}</div></div><button onClick={() => setIsHistoryModalOpen(false)} className="w-full py-4 bg-gray-100 text-gray-500 rounded-2xl font-black text-xs hover:bg-gray-200 transition-all tracking-tight shadow-sm">Fechar Histórico</button></div></Modal>)}
            {isConfirmActionModalOpen && pendingAction && (<Modal title="Confirmar operação" onClose={() => setIsConfirmActionModalOpen(false)} maxWidth="max-w-sm"><div className="text-center p-4 space-y-6"><div className="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-indigo-50 text-indigo-600"><ExclamationTriangleIcon className="w-10 h-10" /></div><div className="space-y-2"><h3 className="text-lg font-bold text-gray-900 dark:text-white">Confirmar ação?</h3><p className="text-xs text-gray-500 font-medium">{pendingAction.type === 'execute' ? 'Deseja marcar como concluída?' : 'Deseja remover este agendamento?'}</p></div><div className="flex gap-4"><button onClick={() => setIsConfirmActionModalOpen(false)} className="flex-1 py-3 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-xl font-bold text-sm text-gray-500">Não</button><button onClick={executePendingAction} disabled={isSaving} className={`flex-1 py-3 rounded-xl font-bold text-sm shadow-lg text-white ${pendingAction.type === 'delete' ? 'bg-red-600' : 'bg-indigo-600'}`}>Sim, confirmar</button></div></div></Modal>)}
