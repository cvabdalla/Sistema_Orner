...
const FormLabel: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <label className="block text-[11px] font-medium text-gray-500 dark:text-gray-400 mb-1 ml-1">{children}</label>
);
...
const SectionHeader: React.FC<{ icon: React.ReactElement<any>; title: string; color?: string; rightElement?: React.ReactNode }> = ({ icon, title, color = "bg-indigo-600", rightElement }) => {
    ...
    return (
        <div className="flex items-center justify-between mb-3 pb-1.5 border-b border-gray-100 dark:border-gray-700/60">
...
const LavagemPage: React.FC<{ currentUser: User }> = ({ currentUser }) => {
    ...
    const inputClass = "w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 py-1.5 px-3 text-xs font-semibold text-gray-700 dark:text-white outline-none focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-950/15 focus:border-indigo-500 transition-all shadow-sm";

    return (
        <div className="space-y-6 animate-fade-in pb-24 font-sans">
            <header className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-gray-200/80 dark:border-gray-700/60 gap-5">
                <div className="flex items-center gap-4 flex-1">
                    <div className="p-3 bg-cyan-600 text-white rounded-xl shadow-sm"><SparklesIcon className="w-6 h-6" /></div>
                    <div>
                        <h2 className="text-xl font-bold text-gray-800 dark:text-white tracking-tight">Gestão de Lavagens</h2>
                        <p className="text-xs text-gray-500 font-medium mt-0.5">Recorrência solar inteligente</p>
                    </div>
                </div>
                <div className="bg-indigo-50/60 dark:bg-indigo-950/20 p-3.5 rounded-xl border border-indigo-200/50 dark:border-indigo-900/30 flex items-center gap-3.5 shadow-sm min-w-[240px]">
                    <div className="p-2 bg-indigo-600 text-white rounded-lg shadow-sm"><DollarIcon className="w-5 h-5" /></div>
                    <div>
                        <p className="text-[10px] font-semibold text-indigo-500 dark:text-indigo-400 leading-none mb-1">{usePeriodFilter ? 'Arrecadado no período' : `Arrecadado em ${new Date().getFullYear()}`}</p>
                        <p className="text-lg font-bold text-indigo-700 dark:text-indigo-300 leading-none">{formatCurrency(totalArrecadado)}</p>
                        <p className="text-[9px] text-indigo-400 font-medium mt-1">* Histórico completo da lista atual</p>
                    </div>
                </div>
                <div className="flex flex-wrap justify-center gap-2.5">
                    <button onClick={() => { setEditingPackage(null); setPackageForm({ name: '', color: PRESET_COLORS[0].hex, wash_qty: 4, price_per_plate: 0, is_negotiated: false, is_courtesy: false }); setIsPackageModalOpen(true); }} className="flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-200 border border-gray-200/60 dark:border-gray-700 rounded-lg font-semibold text-xs transition-all">Planos</button>
                    <button onClick={() => { setEditingClient(null); setClientForm({name:'', phone:'', cep:'', address:'', address_number:'', complement:'', city:'', plates_count:0, installation_end_date: '', observations: ''}); setIsClientModalOpen(true); }} className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-700 rounded-lg font-semibold text-xs transition-all">Novo cliente</button>
                    <button onClick={() => handleOpenLaunchModal()} className="flex items-center gap-2 px-5 py-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded-lg font-semibold text-xs shadow-sm transition-all active:scale-95">Lançar serviço</button>
                </div>
            </header>

            <div className="flex flex-col gap-3">
                <div className="flex bg-gray-100/80 dark:bg-gray-800/80 p-1 rounded-xl w-fit gap-1 border border-gray-200/50 dark:border-gray-700/50">
                    <button onClick={() => setActiveTab('timeline')} className={`flex items-center gap-2 px-4 py-1.5 rounded-lg font-semibold text-xs transition-all ${activeTab === 'timeline' ? 'bg-white dark:bg-gray-700 text-cyan-700 dark:text-cyan-300 shadow-sm border border-gray-200/60 dark:border-gray-600' : 'text-gray-500 hover:text-gray-800 dark:text-gray-400'}`}><CalendarIcon className="w-4 h-4" /> Timeline de Serviços</button>
                    <button onClick={() => setActiveTab('oportunidades')} className={`flex items-center gap-2 px-4 py-1.5 rounded-lg font-semibold text-xs transition-all relative ${activeTab === 'oportunidades' ? 'bg-amber-500 text-white shadow-sm' : 'text-gray-500 hover:text-gray-800 dark:text-gray-400'}`}><TrendUpIcon className="w-4 h-4" /> Carteira de Oportunidades {hotCount > 0 && (<span className="ml-1 px-1.5 py-0.5 rounded-full bg-red-500 text-[10px] font-bold text-white shadow-sm">{hotCount}</span>)}</button>
                    <button onClick={() => setActiveTab('arquivados')} className={`flex items-center gap-2 px-4 py-1.5 rounded-lg font-semibold text-xs transition-all ${activeTab === 'arquivados' ? 'bg-slate-700 text-white shadow-sm' : 'text-gray-500 hover:text-gray-800 dark:text-gray-400'}`}><ArchiveIcon className="w-4 h-4" /> Arquivados {archivedCount > 0 && (<span className="ml-1 text-[10px] bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-200 px-1.5 py-0.5 rounded-full font-semibold">{archivedCount}</span>)}</button>
                </div>
                <div className="bg-white dark:bg-gray-800 p-3.5 rounded-2xl shadow-sm border border-gray-200/80 dark:border-gray-700/60 flex flex-col lg:flex-row items-center gap-3"><div className="relative flex-1 w-full"><SearchIcon className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><input type="text" placeholder="Buscar cliente por nome..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-3 py-1.5 rounded-lg bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-xs font-semibold outline-none focus:ring-2 focus:ring-cyan-500/20 transition-all" /></div><div className="flex flex-wrap items-center gap-3 w-full lg:w-auto"><div className="min-w-[140px]"><FormLabel>Situação</FormLabel><select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-1.5 text-xs font-semibold dark:text-white outline-none appearance-none cursor-pointer"><option value="Todos">Todas situações</option><option value="Oportunidade 1 Ano">Oportunidade 1 Ano 🔥</option><option value="Atrasados">Em atraso ⚠️</option><option value="Com agendamentos">Com agendamentos</option><option value="Sem Agendamento">Sem agendamento</option><option value="Ciclo Finalizado">Ciclo Finalizado</option></select></div><div className="flex items-end gap-2 bg-gray-50 dark:bg-gray-900/40 p-1.5 rounded-xl border border-gray-200/80 dark:border-gray-700/60"><div><FormLabel>Período</FormLabel><select value={usePeriodFilter ? 'enabled' : 'disabled'} onChange={e => setUsePeriodFilter(e.target.value === 'enabled')} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-2.5 py-1 text-xs font-semibold outline-none cursor-pointer shadow-sm"><option value="disabled">Desativado</option><option value="enabled">Habilitado</option></select></div><div className={`flex items-center gap-2 transition-opacity ${usePeriodFilter ? 'opacity-100' : 'opacity-30 pointer-events-none'}`}><div className="relative"><CalendarIcon className="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" /><input type="date" value={periodStart} onChange={e => setPeriodStart(e.target.value)} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg pl-7 pr-2 py-1 text-xs font-semibold outline-none shadow-sm" /></div><span className="text-gray-300 font-bold">-</span><div className="relative"><CalendarIcon className="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" /><input type="date" value={periodEnd} onChange={e => setPeriodEnd(e.target.value)} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg pl-7 pr-2 py-1 text-xs font-semibold outline-none shadow-sm" /></div></div></div></div></div>
            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ml-0 lg:ml-16">
                                {group.items.map(({ client, info }) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            {hasOverdue && (<div className="absolute top-0 right-0 bg-red-500 text-white px-3 py-1 rounded-bl-xl text-[8px] font-black tracking-widest animate-pulse z-10 shadow-sm">Atrasado</div>)}
                                                ...
                                                {(info.allScheduled.length > 0 || info.pendingToScheduleCount > 0) ? (
                                                    <div style={{ backgroundColor: packageColor }} className="rounded-2xl text-white shadow-lg shadow-black/5 overflow-hidden">
                                                        <div className="bg-black/10 px-4 py-2 border-b border-white/10 flex justify-between items-center">
                                                            ...
                                                            <div className="flex items-center gap-1.5">
                                                                <span className="text-[7px] font-bold opacity-60">Progresso:</span>
            ...
            {isPackageModalOpen && (
                <Modal title="Gerenciar planos" onClose={() => { setIsPackageModalOpen(false); setEditingPackage(null); }} maxWidth="max-w-2xl">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-2">
                        <div className="space-y-4">
                            <h4 className="text-[10px] font-black text-gray-400 border-b pb-2">Planos cadastrados</h4>
                        ...
                        <form onSubmit={handleSavePackage} className="space-y-4 bg-gray-50 dark:bg-gray-900/40 p-5 rounded-2xl border">
                            <h4 className="text-[10px] font-black text-indigo-600">{editingPackage ? 'Editar plano' : 'Novo plano'}</h4>
                            <div>
                            ...
                            <button type="submit" disabled={isSaving} className="w-full py-3 bg-indigo-600 text-white rounded-xl font-black text-[10px] shadow-lg tracking-widest hover:bg-indigo-700 transition-all">
                        ...
                        {launchServiceForm.clientId && launchServiceForm.packageId && (() => {
                            ...
                            return (
                                <div className="p-4 bg-indigo-50 dark:bg-indigo-900/30 rounded-2xl border-2 border-indigo-100 dark:border-indigo-800 animate-fade-in">
                                    <h4 className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 tracking-widest mb-3 flex items-center gap-2">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="py-2">
                                            <label className="block text-[10px] font-black text-indigo-500 dark:text-indigo-400 mb-1 ml-0.5">Taxa desloc. (Opcional)</label>
                                            <div className="relative">
                                        ...
                                        <div className="pt-2 border-t border-indigo-200 dark:border-indigo-700 flex justify-between items-center">
                                            <span className="text-[12px] font-black text-indigo-700 dark:text-indigo-300">Investimento total</span>
