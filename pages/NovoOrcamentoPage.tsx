...
import { CalculatorIcon, SaveIcon, AddIcon, TrashIcon, EditIcon, CheckCircleIcon, DollarIcon, CubeIcon, ArrowLeftIcon, PlusIcon, XCircleIcon } from '../assets/icons';
...
const NovoOrcamentoPage = ({ setCurrentPage, orcamentoToEdit, clearEditingOrcamento, currentUser }: NovoOrcamentoPageProps): React.ReactElement => {
    ...
    // Estado local para o formulário de item fora do estoque
    const [manualItemForm, setManualItemForm] = useState({ name: '', cost: 0 });

    // Determina se o formulário está em modo somente leitura
    const isReadOnly = orcamentoToEdit?.status === 'Aprovado';

    // Carrega custo padrão de instalação das configurações se houver
    ...
    const initialFormState = {
        ...
        // IDs de itens do estoque adicionados manualmente que não são fixos
        manualStockItemIds: [] as string[],
        // Itens que não existem no estoque (digitados manualmente)
        offStockItems: [] as { id: string, name: string, cost: number, qty: number, markup: number }[],
        // Armazena dados dos itens de ESTOQUE (fixos ou manuais): { [id]: { qty, cost, markup } }
    ...
    const fixedStockItems = useMemo(() => allStockItems.filter(i => i.isFixedInBudget), [allStockItems]);
    
    // Itens que aparecem na tabela: os fixos + os selecionados manualmente (do estoque)
    const selectedStockTableItems = useMemo(() => {
        ...
        return [...fixedStockItems, ...manualItems];
    }, [allStockItems, fixedStockItems, formState.manualStockItemIds]);

    // Itens disponíveis para adicionar (não são fixos e ainda não foram adicionados do estoque)
    const availableStockToAdd = useMemo(() => {
        const manualIds = formState.manualStockItemIds || [];
        return allStockItems.filter(i => !i.isFixedInBudget && !manualIds.includes(String(i.id)));
    }, [allStockItems, formState.manualStockItemIds]);
    ...
    useEffect(() => {
        ...
        if (orcamentoToEdit) {
            ...
            if (orcamentoToEdit.variants && orcamentoToEdit.variants.length > 0) {
                loadedVariants = orcamentoToEdit.variants.map(v => ({
                    ...
                    formState: {
                        ...initialFormState,
                        ...v.formState,
                        dataOrcamento: isReadOnly ? v.formState.dataOrcamento : today
                    }
                }));
                ...
                loadedVariants = [{
                    ...
                    formState: { 
                        ...
                        dataOrcamento: isReadOnly ? orcamentoToEdit.formState.dataOrcamento : today,
            ...
            if (loadedVariants.length > 0) {
                ...
                setFormState(firstVariant.formState);
                setCalculated(firstVariant.calculated || {});
            }
        } else {
    ...
    }, [orcamentoToEdit, isReadOnly]);

    useEffect(() => {
        const activeVariant = variants.find(v => v.id === activeVariantId);
        if (activeVariant && JSON.stringify(activeVariant.formState) !== JSON.stringify(formState)) {
             setFormState(activeVariant.formState);
             if (activeVariant.calculated) {
    ...
    const updateVariantsWithFormState = (newState: any) => {
        ...
        setVariants(prev => prev.map(v => 
            ...
            : { ...v, 
                formState: { 
                    ...v.formState, 
                    dataOrcamento: newState.dataOrcamento !== formState.dataOrcamento ? newState.dataOrcamento : v.formState.dataOrcamento,
                    nomeCliente: newState.nomeCliente !== formState.nomeCliente ? newState.nomeCliente : v.formState.nomeCliente,
                    fornecedor: newState.fornecedor !== formState.fornecedor ? newState.fornecedor : v.formState.fornecedor,
                } 
            }
        ));
    }
    ...
    const addStockManualItem = (itemId: string) => {
        ...
        const manualIds = [...(formState.manualStockItemIds || []), itemId];
        const currentData = formState.fixedItemsData || {};
        
        const updatedFormState = {
            ...formState,
            manualStockItemIds: manualIds,
            fixedItemsData: {
                ...currentData,
                [itemId]: { qty: 0, cost: stockItem.averagePrice || 0, markup: 0 }
            }
        };
        updateVariantsWithFormState(updatedFormState);
    ...
    const removeStockManualItem = (itemId: string) => {
        if (isReadOnly) return;
        const manualIds = (formState.manualStockItemIds || []).filter(id => id !== itemId);
        const currentData = { ...(formState.fixedItemsData || {}) };
    ...
    useEffect(() => {
        ...
        // Cálculo da estrutura baseada nos itens do estoque (fixos + manuais)
        ...
        // Cálculo da estrutura baseada nos itens FORA do estoque
        ...
        const nfServicoValor = roundUp(despesasNotaCF * (formState.nfServicoPerc / 100));
    ...
    }, [formState, activeVariantId, selectedStockTableItems]);
    ...
    const executeSave = async () => {
        ...
        try {
            ...
            // Garantimos que a mensagem exibida seja uma string
            const errorMsg = e?.message || 'Erro inesperado ao salvar no banco.';
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                ...
                {isReadOnly ? (
                    ...
                    <button 
                        ...
                        disabled={isSaving}
                        className={`w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 ${isSaving ? 'bg-gray-400' : 'bg-green-600 hover:bg-green-700'} text-white rounded-lg font-semibold shadow-lg transition-all`}
                    >
                        <SaveIcon className="w-5 h-5" /> {isSaving ? 'Salvando...' : (orcamentoToEdit ? 'Atualizar projeto' : 'Salvar projeto')}
                    </button>
                )}
            </div>
                ...
                <div className="lg:col-span-8 space-y-6">
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Data do orçamento</label><input type="date" name="dataOrcamento" value={formState.dataOrcamento} readOnly className="w-full rounded-lg border-gray-300 dark:border-gray-600 bg-gray-200 dark:bg-gray-700 p-2.5 text-sm outline-none text-gray-500 dark:text-gray-400 font-bold cursor-not-allowed opacity-80" /></div>
                            <div><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Nome do cliente</label><input type="text" name="nomeCliente" value={formState.nomeCliente} onChange={handleInputChange} disabled={isReadOnly} className={`w-full rounded-lg border-gray-300 dark:border-gray-600 ${isReadOnly ? 'bg-gray-50 cursor-not-allowed' : 'bg-yellow-100 dark:bg-yellow-900/30'} p-2.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none text-gray-900 dark:text-white font-bold`} placeholder="Ex: João Silva" /></div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                            <div><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Fornecedor</label><input type="text" name="fornecedor" value={formState.fornecedor} onChange={handleInputChange} disabled={isReadOnly} className={`w-full rounded-lg border-gray-300 dark:border-gray-600 ${isReadOnly ? 'bg-gray-50 cursor-not-allowed' : 'bg-yellow-100 dark:bg-yellow-900/30'} p-2.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none text-gray-900 dark:text-white font-bold`} placeholder="Ex: Aldo Solar" /></div>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                        <h3 className="text-sm font-bold text-gray-900 dark:text-white tracking-wide border-b border-gray-100 dark:border-gray-700 pb-3 mb-4 flex items-center gap-2"><span className="w-1 h-4 bg-orange-500 rounded-full"></span> Custos de instalação e terceiros</h3>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                <div className="flex justify-between items-center mb-1"><label className="block text-xs font-medium text-gray-500 dark:text-gray-400">Instalação - Placas</label><span className="text-[10px] font-bold text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded">Total: {formatCurrency(formState.terceiroInstalacaoQtd * formState.terceiroInstalacaoCusto)}</span></div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                    {/* Renderiza itens de ESTOQUE selecionados (fixos ou manuais) */}
                                    ...
                                    {/* Renderiza itens FORA do estoque (Manuais digitados) */}
            ...
            {/* Modal para adicionar itens (Estoque ou Manual) */}
            {isAddItemModalOpen && (
            ...
            {isModalOpen && (<Modal title="Ajustar margem de serviço" onClose={() => setModalOpen(false)}><div className="space-y-4"><label htmlFor="desiredMargin" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Margem líquida final desejada (%)</label><div className="relative"><input type="number" id="desiredMargin" value={desiredMargin} onChange={(e) => setDesiredMargin(e.target.value)} className="w-full text-right rounded-md border-gray-300 dark:border-gray-600 bg-yellow-50 dark:bg-yellow-900/10 font-bold text-gray-900 dark:text-yellow-100 focus:ring-indigo-500 focus:border-indigo-500 pr-8 p-2" placeholder="ex: 60" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 font-bold">%</span></div><div className="flex justify-end pt-4"><button onClick={handleMarginCalculation} className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Calcular</button></div></div></Modal>)}
