...
const App: React.FC = () => {
  ...
  // ID Padrão do administrador
  const ADMIN_PROFILE_ID = '001';

  // Sincronização do Modo Noturno
  useEffect(() => {
  ...
  const fetchPermissions = async (profileId: string) => {
    try {
      ...
      if (!profile) {
          profile = MOCK_PROFILES.find(p => String(p.id) === String(profileId));
      }
  ...
  // Verifica sessão ao carregar - OBRIGATÓRIO LOGIN
  useEffect(() => {
  ...
  // Sincroniza o usuário e permissões ao mudar de página
  useEffect(() => {
  ...
  const renderPage = () => {
    ...
    if (!currentUser.active) {
        return (
            <div className="flex flex-col items-center justify-center h-full space-y-6 animate-fade-in text-center px-4">
                ...
                <button onClick={handleLogout} className="text-xs text-indigo-600 font-bold hover:underline">Voltar para o início</button>
            </div>
        );
    }
    ...
    switch (currentPage) {
      ...
      case 'FINANCEIRO_BANCOS':
        return <FinanceiroPage view="bancos" currentUser={currentUser} />;
      case 'ESTOQUE_VISAO_GERAL':
        return <EstoquePage view="visao_geral" setCurrentPage={handleSetCurrentPage} currentUser={currentUser} userPermissions={userPermissions} />;
      case 'ESTOQUE_NOVO_PRODUTO':
        return <EstoquePage view="cadastro" setCurrentPage={handleSetCurrentPage} currentUser={currentUser} userPermissions={userPermissions} />;
      case 'ESTOQUE_COMPRAS':
        return <EstoquePage view="compras" setCurrentPage={handleSetCurrentPage} currentUser={currentUser} userPermissions={userPermissions} />;
      ...
      case 'USUARIOS_PERFIL':
        return <UsuariosPage view="perfil" currentUser={currentUser} />;
      default:
        return <WelcomePage currentUser={currentUser} userPermissions={userPermissions} onNavigate={handleSetCurrentPage} />;
    }
  };
  ...
  const getPageTitle = () => {
    ...
    const titles: Record<string, string> = {
      ...
      RELATORIOS_VISAO_GERAL: 'Resumo Reembolso',
      RELATORIOS_NOVO: editingReport ? 'Editar Reembolso' : 'Novo Reembolso',
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 print:block print:h-auto print:overflow-visible transition-colors duration-300">
      <Sidebar 
